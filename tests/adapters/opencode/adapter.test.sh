...
EOF
  ...
  [[ -f "$dst/.opencode/plugins/brc-hooks.js" ]] || { echo "brc-hooks.js not generated"; result=1; }
  rm -rf "$src" "$dst"
  return $result
}
...
EOF
  ...
  adapter_translate_hooks "$src/hooks" "$dst"
  local plugin="$dst/.opencode/plugins/brc-hooks.js"
  local result=0
...
test_oc_translate_hooks_no_hooks_dir_is_noop() {
  ...
  adapter_translate_hooks "$src/hooks" "$dst"
  local result=0
  [[ ! -f "$dst/.opencode/plugins/brc-hooks.js" ]] || { echo "plugin should not exist when no hooks"; result=1; }
  rm -rf "$src" "$dst"
  return $result
}
...
EOF
  ...
  [[ -f "$dst/.opencode/plugins/brc-hooks.js" ]]            || { echo "plugin missing"; result=1; }
