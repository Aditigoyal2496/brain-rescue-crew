...
5. **Update the registry.** If the change affects the agent's description, triggers, or capabilities, update the corresponding row in `.platform/references/agents-registry.md`. Custom agent rows live between the `<!-- BRC:CUSTOM_AGENTS_START -->` and `<!-- BRC:CUSTOM_AGENTS_END -->` markers — edit only within that block.
