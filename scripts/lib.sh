#!/usr/bin/env bash
# =============================================================================
# Brain Rescue Crew :: Shared library
...
# Inserts <content> immediately after the BRC:CUSTOM_AGENTS_START line in dst.
...
_insert_after_start_marker() {
  ...
  local start_line
  start_line=$(grep -n '<!-- BRC:CUSTOM_AGENTS_START -->' "$dst" | head -1 | cut -d: -f1)
...
# Copies src to dst, preserving content between <!-- BRC:CUSTOM_AGENTS_START -->
# and <!-- BRC:CUSTOM_AGENTS_END --> markers from the existing dst.
#
# Handles three cases:
...
merge_marked_file() {
  ...
  grep -q '<!-- BRC:CUSTOM_AGENTS_START -->' "$src" 2>/dev/null && src_has_markers=1
  grep -q '<!-- BRC:CUSTOM_AGENTS_START -->' "$dst" 2>/dev/null && dst_has_markers=1
  ...
  if [[ $dst_has_markers -eq 0 ]]; then
    ...
    if [[ -n "$custom_rows" ]]; then
      ...
      warn "Migrated legacy custom agents in $(basename "$dst") to BRC marker format"
    fi
    return 0
  fi
  ...
  local saved_content
  saved_content=$(awk \
    '/<!-- BRC:CUSTOM_AGENTS_START -->/{f=1; next}
     /<!-- BRC:CUSTOM_AGENTS_END -->/{f=0}
     f{print}' \
    "$dst")
...
# ── Manifest helpers ──────────────────────────────────────────────────────────
# Single unified manifest at $VAULT_DIR/.{framework}/.brc-manifest
...
manifest_read() {
  local section="$1"
  local file="${PLATFORM_VAULT_DIR:-$VAULT_DIR/.claude}/.brc-manifest"
  [[ -f "$file" ]] || return 0
  awk -v sec="[$section]" '
    $0 == sec         { found=1; next }
...
manifest_migrate() {
  ...
  warn "Migrating legacy manifests to unified .brc-manifest..."
  ...
  success "Manifest migrated to $(basename "$_fw_dir")/.brc-manifest"
}
...
# contain user-added content between BRC markers. These use merge_marked_file
...
print_banner() {
  ...
  echo ""
  echo -e "${BOLD}╔══════════════════════════════════════════╗${NC}"
  echo -e "${BOLD}║  Brain Rescue Crew :: ${title}${NC}"
  echo -e "${BOLD}╚══════════════════════════════════════════╝${NC}"
  echo ""
}
